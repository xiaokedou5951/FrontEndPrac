...
export function ListingsTable({ listings, metadata, onSelectListing }: Props) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-100 text-left text-gray-500">
              ...
              <th className="px-2 py-1 text-right">优惠价(50%)</th>
            </tr>
          </thead>
          <tbody>
            {listings.map((l) => (
              ...
              >
                ...
                <td className="px-2 py-1 text-right font-mono text-emerald-600">
                  {formatTokenAmount(l.price / 2n, decimals, 4)} {symbol}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {onSelectListing && (
        <p className="mt-2 text-xs text-gray-400">点击行可自动填入优惠购买的 listingId</p>
      )}
    </div>
  );
}
