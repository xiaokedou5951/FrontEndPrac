...
import { erc721Abi } from "@/contracts/nftMarketPermitAbi";
...
export function ApproveNFTCard() {
  ...
  const canSubmit =
    !!account && !isPending && addressValid && !!nftMarketPermitAddress;
  ...
  const handleApprove = async () => {
    if (!account || !nftMarketPermitAddress) return;
    ...
    try {
      await writeContract({
        ...
        args: [nftMarketPermitAddress, true],
      });
    } catch (e) {
