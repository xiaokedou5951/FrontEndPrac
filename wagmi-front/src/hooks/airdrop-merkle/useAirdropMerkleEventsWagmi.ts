...
import type { MarketLog, MarketEventName } from "@/components/airdrop-merkle/types";
  ...
  switch (log.eventName) {
    ...
    case "NFTClaimed":
      return {
        ...
        nftContract: getAddress(log.args, "nftContract"),
        tokenId: getBigInt(log.args, "tokenId"),
        paidAmount: getBigInt(log.args, "paidAmount"),
      };
    case "MerkleRootUpdated":
      return {
        ...base,
        newRoot: (log.args.newRoot as `0x${string}` | undefined),
      };
...
function formatLogForConsole(log: MarketLog): string {
  ...
  if (log.paidAmount !== undefined) parts.push(`paid=${log.paidAmount.toString()}`);
  if (log.newRoot) parts.push(`newRoot=${log.newRoot}`);
  parts.push(`block=${log.blockNumber?.toString() ?? "?"}`);
  parts.push(`tx=${log.txHash}`);
  return parts.join(" ");
}

export function useAirdropMerkleEventsWagmi(enabled = true) {
  ...
  const addLogs = useCallback((incoming: MarketLog[]) => {
    ...
    fresh.forEach((l) => {
      ...
      console.log(`[AirdropMerkle][${l.eventName}] ${formatLogForConsole(l)}`, l);
    });
