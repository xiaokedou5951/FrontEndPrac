...
import type { MarketLog, MarketEventName } from "@/components/nftmarket-permit/types";
...
export function useNFTMarketPermitEventsWagmi(enabled = true) {
  ...
  const {
    ...
    fromBlock: 0n,
    query: {
      enabled: enabled && !!nftMarketPermitAddress,
    },
  });
  ...
  const addLogs = useCallback((incoming: MarketLog[]) => {
    ...
    fresh.forEach((l) => {
      ...
      console.log(`[NFTMarketPermit][${l.eventName}] ${formatLogForConsole(l)}`, l);
    });
